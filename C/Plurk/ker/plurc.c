	...
	if (argc==3) {
		if (!strcmp(argv[1], "add")) {
			...
			*/
            ...
            printf("You are gonna delete the plurk with plurk_id: %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            }
            if (!plurk_timeline_plurkDelete(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk del error\n");
                rc = -1;
                goto out;
            }
            if (!plurk_logout(ph)) {
			...
			*/
		...
		} else if (!strcmp(argv[1], "getPlurk")) {
            printf("You wanna get get plurk id : %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_getPlurk(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk getPlurk error\n");
                rc = -1;
                goto out;
            }
        ...
        } else if (!strcmp(argv[1], "mutePlurks")) {
            printf("You wanna mutePlurks ids : %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_mutePlurks(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk mutePlurks error\n");
                rc = -1;
                goto out;
            }
        ...
        }  else if (!strcmp(argv[1], "unmutePlurks")) {
            printf("You wanna unmutePlurks ids : %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_unmutePlurks(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk unmutePlurks error\n");
                rc = -1;
                goto out;
            }
        ...
        }  else if (!strcmp(argv[1], "favoritePlurks")) {
            printf("You wanna favoritePlurks ids : %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_favoritePlurks(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk favoritePlurks error\n");
                rc = -1;
                goto out;
            }
        ...
        } else if (!strcmp(argv[1], "unfavoritePlurks")) {
            printf("You wanna unfavoritePlurks ids : %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_unfavoritePlurks(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk unfavoritePlurks error\n");
                rc = -1;
                goto out;
            }
        ...
        }  else if (!strcmp(argv[1], "markAsRead")) {
            printf("You wanna markAsRead ids : %s\n", argv[2]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_markAsRead(ph, argv[2])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk markAsRead error\n");
                rc = -1;
                goto out;
            }
		...
		if (!strcmp(argv[1], "resps_radd")) {
			...
			*/
		...
		} else if (!strcmp(argv[1], "plurkEdit")) {
            printf("You wanna edit plurk[%s]  to %s\n", argv[2], argv[3]);
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_plurkEdit(ph, argv[2], argv[3])) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk edit error\n");
                rc = -1;
                goto out;
            }
		...
		if (!strcmp(argv[1], "oprofile_get")) {
			...
			*/
		...
		} else if (!strcmp(argv[1], "unreadCount_get")) {
            printf("You wanna get unread plurk count \n");
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_polling_getUnreadCount(ph)) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk get UnreadCount error\n");
                rc = -1;
                goto out;
            }
        ...
        } else if (!strcmp(argv[1], "getUnreadPlurks")) {
            printf("You wanna get unread plurks \n");
            if (plurk_login(ph, PLURK_USERNAME, PLURK_PASSWORD)) {
                fprintf(stderr, "Plurk login error\n");
                rc = -1;
                goto out;
            ...
            if (!plurk_timeline_getUnreadPlurks(ph)) {
                jo = json_create_obj(ph->body);
                if (jo) {
                    json_print_obj(jo, 0);
                    json_free_obj(jo);
                ...
                fprintf(stderr, "Plurk getUnreadPlurks error\n");
                rc = -1;
                goto out;
            }
