...
findfiles('/home/xatierlike/');

sub findfiles {
  my $limit = 3600;
  chdir "$_[0]" || die "fuck it :$!";   # cd to the directory
  my @dir = ();
  my @file = ();
  while (my $f = <*>) {                 # get all the files in current dir
    if (-d $f) {                        # I'm a directory
      push @dir, ($f);
    }
    elsif (-f $f){                      # I'm a file
      push @file, $f;
    }
  }

#  print "DBG ==>  Now in dir:" . `pwd` . scalar (@file) . "files here\n";

  for my $ff (@file) {
     if ((time - (stat $ff)[9]) > $limit) {
       print ("$ff modified at : ");
       my @modtime =  reverse ((localtime((stat $ff)[9]))[0 .. 5 ]);
       $modtime[0] += 1900;
       print (join '/', @modtime[0 .. 2], " ", join ':', @modtime[3 .. 5], "\n");
     }
  }

  for (@dir) {                         # recursively deal with all directorys
#    print "DBG ==>  will find in $_ ...\n";
    findfiles($_);
    chdir '..';                        # go back
  }
}
...
foreach $dir (@ARGV) {
   ...
   foreach $level (@levels) {
       printf("%4d - %4d %8d Kb %5d %8d Kb %5d\n",
              ...
              $mfiles{$level} , $accessed{$level}/1024,$afiles{$level});
